...
files = [
    ...
    'Data/greenery_pending.csv',
    'Data/roi_department_report_20260209_112210(MIS Report).csv'
]

# We use regexes that gracefully handle trailing spaces or missing spaces
replacements = [
    (re.compile(r'\brmhs upto 10\s*mt\b', re.IGNORECASE), 'Raw Material Handling System'),
    (re.compile(r'\brmhs upto 5\s*mt\b', re.IGNORECASE), 'Raw Material Handling System'),
    (re.compile(r'\brmhs upto 7\s*mt\b', re.IGNORECASE), 'Raw Material Handling System')
]
